...
// RangeOpt stores either a single float, in which case the value is stored in min and IsRange is false,
...
type RangeOptJSON struct {
	Value string `json:"value"`
}

func (o *RangeOpt) MarshalJSON() ([]byte, error) {
	value := ""
	if o.Min == o.Max {
	...
	return json.Marshal(&RangeOptJSON{
		Value: value,
	})
}

func (o *RangeOpt) UnmarshalJSON(b []byte) error {
	var inc RangeOptJSON
...
// Initialize sets up the RangeOpt from string value
func (o *RangeOpt) Initialize(value string) error {
	var err, err2 error
